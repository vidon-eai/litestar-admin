...
export class ApiError extends Error {
  ...
  constructor(
    ...
    if (rawData && typeof rawData === "object") {
      const err = rawData as ApiErrorResponseBody
      this.code = err.code
...
export async function fetcher<T>(
  ...
  if (!response.ok) {
    ...
    let errData: ApiErrorResponseBody | null = null
    try {
      errData = (await response.json()) as ApiErrorResponseBody
    } catch {
      errData = null
    }

    // 自動解析 detail 陣列內的錯誤訊息
    let parsedMessage = ""
    if (errData?.detail) {
      if (Array.isArray(errData.detail) && errData.detail.length > 0) {
        parsedMessage = errData.detail.map((d) => d.message).join(", ")
    ...
    throw new ApiError(response.status, finalMessage, errData)
  }
